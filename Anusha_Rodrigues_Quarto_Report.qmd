...
```{python}
#| colab: {base_uri: https://localhost:8080/, height: 885}
...
```{python}
#| label: country-dropdown-year-slider-altair
#| fig-cap: Dropdown for Country and Year Slider (Altair)
#| echo: true
#| message: false
#| warning: false
...
# Group
country_mortality = cleaned_data.groupby(['country', 'year'])['proportion_of_schools_with_no_hygiene_service'].mean().reset_index()

# Create dropdown selection (new altair 5 style)
country_dropdown = alt.binding_select(options=sorted(country_mortality['country'].unique()), name='Select Country: ')
country_select = alt.selection_point(fields=['country'], bind=country_dropdown, value=[{'country': 'India'}])

# Create line chart
line_chart = (
    alt.Chart(country_mortality)
    .mark_line(point=True)
    .encode(
        x=alt.X('year:O', title='Year'),
        y=alt.Y('proportion_of_schools_with_no_hygiene_service:Q', title='No Hygiene Service (%)'),
        tooltip=['country:N', 'year:O', 'proportion_of_schools_with_no_hygiene_service:Q'],
    ...
    .properties(
        width=900,
        height=500,
        title="No Hygiene Service in Schools Over Time"
    )
)
